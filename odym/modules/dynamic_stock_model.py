...
class DynamicStockModel(object):
    ...
    o_c : np.array, optional
        Discrete time series of outflow from stock, by cohort
    s_c : np.array, optional
        Dynamic stock model (stock broken down by year and age- cohort)
    s : np.array, optional
        Discrete time series for stock, total
    lt : dict, optional
        Lifetime distribution. Keys other than 'Type' in this dict that are
        scalar values or length-1 np.arrays are broadcast to the same length as
        *t*.
    pdf : np.array, optional
        Probability density function, distribution of outflow from a specific
        age-cohort
    sf : np.array, optional
        Survival function for different age-cohorts, year x age-cohort table
    """

    def __init__(self, name='DSM', **kwargs):
        """Assign the input data to the instance of the object."""
        self.name = name

        for var in 'i o o_c pdf s s_c sf t'.split():
        ...
        for key, value in lt.items():
            ...
            if isinstance(value, float) or len(value) == 1:
                lt[key] = np.tile(value, len(self.t))
    ...
    def compute_sf(self):
        ...
        (age-cohort) still present at the end of year m (after m-n years). The
        computation is self.sf(m,n) = ProbDist.sf(m-n), where ProbDist is the
        appropriate scipy function for the lifetime model chosen. For lifetimes
        0 the sf is also 0, meaning that the age-cohort leaves during the same
        year of the inflow. The method compute outflow_sf returns an array
        year-by-cohort of the surviving fraction of a flow added to stock in
        year m (aka cohort m) in in year n. This value equals sf(n,m). This is
        the only method for the inflow-driven model where the lifetime
        distribution directly enters the computation. All other stock variables
        are determined by mass balance. The shape of the output sf array is
        NoofYears * NoofYears, and the meaning is years by age-cohorts. The
        method does nothing if the sf alreay exists. For example, sf could be
        assigned to the dynamic stock model from an exogenous computation to
        save time.
        """
        if self.sf:
            return self.sf
        ...
        if self.lt['Type'] == 'Fixed': # fixed lifetime, age-cohort leaves the stock in the model year when the age specified as 'Mean' is reached.
        ...
        elif self.lt['Type'] == 'Normal': # normally distributed lifetime with mean and standard deviation. Watch out for nonzero values
        ...
        elif self.lt['Type'] == 'FoldedNormal': # Folded normal distribution, cf. https://en.wikipedia.org/wiki/Folded_normal_distribution
            for m in range(0, len(self.t)):  # cohort index
                if self.lt['Mean'][m] != 0:  # For products with lifetime of 0, sf == 0
        ...
        elif self.lt['Type'] == 'LogNormal': # lognormal distribution
            ...
            # Weibull distribution with standard definition of scale and shape
            # parameters
            shape = self.lt['Shape']
            scale = self.lt['Scale']
            for m in range(0, len(self.t)):  # cohort index
                ...
                self.sf[m::, m] = weibull_min.sf(
                    np.arange(0, len(self.t) - m),
                    c=shape[m],
                    loc=0,
                    scale=scale[m])
    ...
    def compute_stock_driven_model(self, NegativeInflowCorrect=False):
        ...
        Parameters
        ----------
        NegativeInflowCorrect : bool, optional
            If the stock declines faster than according to the lifetime model,
            this option allows to extract additional stock items. This part was
            contributed by Sebastiaan Deetman, CML Leiden, and adapted by S.P.
            so that the mass balance of the stock fits.

            This method of negative inflow correction is only one of many
            plausible methods of increasing the outflow to keep matching stock
            levels. It assumes that the surplus stock is removed in the year
            that it becomes obsolete. Each cohort loses the same fraction.
            Modellers need to try out whether this method leads to justifiable
        ...
        # Construct the sf of a product of cohort tc remaining in the stock in
        # year t
        self.compute_sf()

        if NegativeInflowCorrect:
            # Determine pdf from sf array for computations below.
        ...
        self.s_c[:, 0] = self.i[0] * self.sf[:, 0]
        self.o_c[0, 0] = self.i[0] - self.s_c[0, 0]

        # all other years, starting in the second year
        for m in range(1, len(self.t)):
            ...
            self.o_c[m, 0:m] = self.s_c[m-1, 0:m] - self.s_c[m, 0:m]

            # 2) Determine inflow from mass balance:
            if not NegativeInflowCorrect:
                # if no correction for negative inflows is made
                if self.sf[m, m] != 0:  # Else, inflow is 0.
                    # Allow for outflow during first year by rescaling with
                    # 1/sf[m,m]
                    self.i[m] = (self.s[m] - self.s_c[m, :].sum()) / \
                        self.sf[m, m]
                # 3) Add new inflow to stock and determine future decay of new
                # age-cohort
                self.s_c[m::, m] = self.i[m] * self.sf[m::, m]
                self.o_c[m, m] = self.i[m] * (1 - self.sf[m, m])
            # 2a) Correct remaining stock in cases where inflow would be
            # negative:
            else:
                # if the stock declines faster than according to the lifetime
                # model, this option allows to extract additional stock items.
                # The negative inflow correction implemented here was developed
                # in a joined effort by Sebastiaan Deetman and Stefan Pauliuk.
                InflowTest = self.s[m] - self.s_c[m, :].sum()
                if InflowTest < 0:
                    # Stock-driven model would yield negative inflow
                    Delta = -1 * InflowTest  # Delta > 0!

                    # Set inflow to 0 and distribute mass balance gap onto
                    # remaining cohorts:
                    self.i[m] = 0
                    if self.s_c[m, :].sum() != 0:
                        Delta_percent = Delta / self.s_c[m, :].sum()
                        # Distribute gap equally across all cohorts (each
                        # cohort is adjusted by the same %, based on surplus
                        # with regards to the prescribed stock)
                        # Delta_percent is a % value <= 100%
                    else:
                        # stock in this year is already zero, method does not
                        # work in this case.
                        Delta_percent = 0
                    # correct for outflow and stock in current and future years
                    # adjust the entire stock AFTER year m as well, stock is
                    # lowered in year m, so future cohort survival also needs
                    # to decrease.

                    # increase outflow according to the lost fraction of the
                    # stock, based on Delta_c
                    self.o_c[m, :] = self.o_c[m, :] + \
                        (self.s_c[m, :] * Delta_percent).copy()
                    # shrink future description of stock from previous
                    # age-cohorts by factor Delta_percent in current AND future
                    # years.
                    self.s_c[m::, 0:m] = self.s_c[m::, 0:m] * \
                        (1 - Delta_percent.copy())
                else:
                    # If no negative inflow would occur
                    if self.sf[m, m] != 0:  # Else, inflow is 0.
                        # Allow for outflow during first year by rescaling with
                        # 1/sf[m,m]
                        self.i[m] = (self.s[m] - self.s_c[m, :].sum()) / \
                            self.sf[m, m]
                    # Add new inflow to stock and determine future decay of new
                    # age-cohort
                    self.s_c[m::, m] = self.i[m] * self.sf[m::, m]
                    self.o_c[m, m] = self.i[m] * (1 - self.sf[m, m])
